...
def test_create_item(client: TestClient, superuser_token_headers: dict[str, str]) -> None:
    ...
    response = client.post(
        f"{settings.API_V1_PREFIX}/{{ cookiecutter.first_model }}/",
...
def test_read_item(client: TestClient, superuser_token_headers: dict[str, str]) -> None:
    ...
    # Read Item
    response = client.get(
        f"{settings.API_V1_PREFIX}/{{ cookiecutter.first_model }}/{created_item['id']}",
        headers=superuser_token_headers,
    )
    assert response.status_code == 200
...
def test_get_item_not_found(client: TestClient, superuser_token_headers: dict[str, str]) -> None:
    ...
    """
    response = client.get(
        f"{settings.API_V1_PREFIX}/{{ cookiecutter.first_model }}/1",
        headers=superuser_token_headers,
    )
...
def test_get_item_forbidden(
    db_with_{{ cookiecutter.first_model }}s: Session, client: TestClient, normal_user_token_headers: dict[str, str]
) -> None:
    """
    Test that a forbidden error is returned.
    """
    response = client.get(
        f"{settings.API_V1_PREFIX}/{{ cookiecutter.first_model }}/5kwf8hFn",
...
def test_superuser_get_all_items(
    db_with_{{ cookiecutter.first_model }}s: Session, client: TestClient, superuser_token_headers: dict[str, str]
) -> None:
    """
...
def test_update_item(client: TestClient, superuser_token_headers: dict[str, str]) -> None:
    ...
    # Update wrong item
    response = client.patch(
        f"{settings.API_V1_PREFIX}/{{ cookiecutter.first_model }}/99999",
        headers=superuser_token_headers,
        json=update_data,
    )
    assert response.status_code == 404
