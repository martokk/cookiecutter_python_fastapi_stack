...
from {{ cookiecutter.project_slug }}.db.init_db import create_all
...
async def test_create_all(tmpdir: str, monkeypatch: MagicMock) -> None:
    ...
    monkeypatch.setattr("{{ cookiecutter.project_slug }}.paths.DATABASE_FILE", db_file)
    ...
    assert "{{ cookiecutter.first_model }}" in tables
