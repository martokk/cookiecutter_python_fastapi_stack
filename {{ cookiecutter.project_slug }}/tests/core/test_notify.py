...
from {{ cookiecutter.project_slug }} import logger, settings
from {{ cookiecutter.project_slug }}.core import notify


@patch("{{ cookiecutter.project_slug }}.settings.TELEGRAM_API_TOKEN", "valid_token")
@patch("{{ cookiecutter.project_slug }}.settings.TELEGRAM_CHAT_ID", 123)
...
@patch("{{ cookiecutter.project_slug }}.settings.TELEGRAM_CHAT_ID", 0)
...
@patch("{{ cookiecutter.project_slug }}.settings.EMAILS_ENABLED", True)
@patch("{{ cookiecutter.project_slug }}.settings.NOTIFY_EMAIL_TO", "test@example.com")
async def test_notify() -> None:
    """
    Test that the notify function calls the telegram and email functions.
    """
    with patch("{{ cookiecutter.project_slug }}.core.notify.send_telegram_message") as mock_telegram:
        with patch("{{ cookiecutter.project_slug }}.core.notify.send_email") as mock_email:
...
@patch("{{ cookiecutter.project_slug }}.settings.TELEGRAM_CHAT_ID", 123)
@patch("{{ cookiecutter.project_slug }}.settings.EMAILS_ENABLED", True)
@patch("{{ cookiecutter.project_slug }}.settings.SMTP_USER", "user")
@patch("{{ cookiecutter.project_slug }}.settings.SMTP_PASSWORD", "password")
...
@patch("{{ cookiecutter.project_slug }}.settings.EMAILS_ENABLED", False)
...
@patch("{{ cookiecutter.project_slug }}.core.notify.get_html_template", "")
async def test_send_test_email() -> None:
    """
    Test that the send_test_email function calls the send_email function.
    """
    with patch("{{ cookiecutter.project_slug }}.core.notify.get_html_template") as mock_get_html_template:
        mock_get_html_template.return_value = ""
        with patch("{{ cookiecutter.project_slug }}.core.notify.send_email") as mock_send_email:
...
@patch("{{ cookiecutter.project_slug }}.settings.NOTIFY_ON_START", True)
async def test_notify_on_start(client: TestClient) -> None:
    ...
    with patch("{{ cookiecutter.project_slug }}.core.notify.get_html_template") as mock_get_html_template:
        mock_get_html_template.return_value = ""
        with patch("{{ cookiecutter.project_slug }}.core.notify.notify") as mock_notify:
            with client as c:
                c.get("/'")
